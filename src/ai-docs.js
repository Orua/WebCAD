...
Shell-only clients can GET /api/agent for compact discovery and use scripts/webcad-agent.mjs (npm run agent:cli --) for docs, sessions, state, calls and verified file transfer. Execution still requires an explicitly opened /agent browser tab; the static default page does not connect to the bridge.

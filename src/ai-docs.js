...
Legacy development transport only: /api/agent and webcad-agent.mjs are not the product AI entrance or sidebar acceptance. The static page exposes window.webcad.api.run and an in-page JSON panel; read automation/quickstart.md.
Read info().capabilities.migratedOperations and the current tool card for strict v2 operations, including multiPocket. Legacy operations remain documented and available through their existing entrypoints; schema metadata is not proof of kernel success.
...
slot.angle is degrees, independent of the sign of cut direction.`,
  'recipes.multi-pocket': `Use getTool({id:"multiPocket"}) and readDocs({docId:"recipes.multi-pocket"}) before execution. Pass one current body ID as refs and params:{depth:0.5,axis:"Z",direction:-1,pockets:[{x:10,y:10,z:3,width:6,height:4},{x:25,y:10,z:3,width:6,height:4,cornerRadius:0.5}]}. Each XYZ is a world cutter start center; width/height directions are X/Y for Z, Y/Z for X, Z/X for Y. Use getState and measure to verify the committed revision and exact solid. Edit the historical feature to adjust every pocket together.`,

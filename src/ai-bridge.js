...
export function connectAI(api,{onStatus=()=>{}}={}) {
 let socket=null,stopped=false,retry=null,sessionId=null,tail=Promise.resolve();
 ...
 function open(){
  ...
  ws.onmessage=event=>{let msg;try{msg=JSON.parse(event.data);}catch{report('error','MCP 桥收到无效消息');return;}
   if(msg.type==='welcome'){sessionId=msg.sessionId;report('connected','MCP 已连接');publish(ws);return;}
   ...
   const run=async()=>{
    ...
    try{
     ...
     const result=msg.command==='get_state'?state:await api.execute(msg.command,msg.args||{},{signal:controller.signal,expectedRevision});
     if(controller.signal.aborted)throw new Error('Operation cancelled; transaction must not commit');
  ...
  ws.onclose=()=>{for(const controller of active.values())controller.abort();sessionId=null;report('disconnected','MCP 已断开；操作已取消，等待重连');if(!stopped)retry=setTimeout(open,2000);};
  ws.onerror=()=>report('error','本机 MCP 服务连接失败');
 }
 const heartbeat=setInterval(()=>publish(),3000);open();
 return {publish,get sessionId(){return sessionId;},disconnect(){stopped=true;clearTimeout(retry);clearInterval(heartbeat);for(const c of active.values())c.abort();socket?.close();}};
}

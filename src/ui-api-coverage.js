...
export const UI_API_ROUTES = Object.freeze({
  ...
  anchorVisibility:{tools:['setView'],method:'setView',usage:'参考锚点呼吸球显隐为本地 UI 偏好，不改变工程、固定原始坐标或几何 revision。'},
  ...
  agentGuide:{tools:['connect','getTools','readDocs'],method:'connect',usage:'connect({queries,includeContracts:true}) 返回当前 canExecute/blockers、requestContext 与完整卡；onboarding 提供首次连接步骤、本地安装包和真实操作路由；readDocs({docId:"api.connection"}) 说明宿主能力检查。knownHashes 只缓存完整工具卡，不缓存实体身份。'},
  logoConverterSettings:{tools:['getLogoConverter','setLogoConverter'],method:'setLogoConverter',usage:'context、url、key；getLogoConverter 读回配置状态。'},
  displayPreferences:{tools:['setDisplayPreferences'],method:'setDisplayPreferences',usage:'渲染设置使用 setDisplayPreferences；LOGO 转化为独立入口。'},
  panels:{tools:['setView'],method:'setView',usage:'panels:{left:boolean,right:boolean}；getState().view.panels 读状态。'},
  moveTool:{tools:['setView','transform'],method:'setView',usage:'setView({context,gizmo:"translate",selectedIds?}) 开启鼠标移动；自动使用实体选择。getState().view.transform 读可拖状态与 blocker；精确移动用 transform 的显式 params/refs，经 execute 或 run 提交。'},
  rotateTool:{tools:['setView','transform'],method:'setView',usage:'setView({context,gizmo:"rotate",selectedIds?}) 开启单对象鼠标旋转。getState().view.transform 读可拖状态；精确旋转用 transform，显式指定角度、轴与中心，不依赖 UI 选择。多对象分别用 run 的 transform 步骤，atomic=false。'},
  copySelection:{tools:['copySelection'],method:'copySelection',usage:'context、bodyIds，读 getState().view.clipboard。'},
  pasteSelection:{tools:['pasteSelection'],method:'pasteSelection',usage:'context、idempotencyKey；整体底面中心放在锚点。'},
  ...
  sketch:{tools:['extrude'],method:'execute',usage:'兼容的轮廓并拉伸入口对应 extrude，传 profile/points/plane 等显式参数。主绘制轮廓入口用 sketchProfile，再单独 profileExtrude；不需要模拟逐点点击。'},
  ...
  profileConstraints:{tools:['profileConstraints','inspectConstraints'],method:'execute',usage:'inspectConstraints({context,bodyId})读真实局部图元、约束图及自由度；profileConstraints新建派生轮廓，继承来源关系。feature.edit修改原constraints数组驱动下游。'},

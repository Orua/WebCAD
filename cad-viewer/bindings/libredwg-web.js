...
class LibreDwgConverter {
  ...
  convertForViewer(data, onBatch, batchSize = 2e3) {
    ...
    for (let i = 0; i < numObjects; i++) {
      ...
      if (fixedtype === Dwg_Object_Type.DWG_TYPE_LAYER) {
        ...
        blocks.push({
          ...
          name,
          handle: attrs.handle,
          basePoint: libredwg.dwg_dynapi_entity_data(tio, "base_pt") ?? { x: 0, y: 0, z: 0 },
          numOwned: libredwg.dwg_dynapi_entity_data(tio, "num_owned") ?? 0
        });
      }
    }
    blocks.sort(
    ...
    const emitBlock = (block) => {
      ...
      if (blockEntityCount === 0 && block.numOwned > 0) {
        const entitiesPtr = libredwg.dwg_dynapi_entity_data(
          block.tio,
          "entities"
        );
        if (entitiesPtr) {
  ...
  convertHeader(data, header) {
    ...
    HEADER_VARIABLES.forEach((name) => {
      ...
      let value = libredwg.dwg_dynapi_header_data(
        data,
        var_name
      );
  ...
  convertBlockRecord(item, obj) {
    ...
    const description = libredwg.dwg_dynapi_entity_data(
      item,
      "description"
    );
    const basePoint = libredwg.dwg_dynapi_entity_data(
      item,
      "base_pt"
    );
    const insertionUnits = libredwg.dwg_dynapi_entity_data(
      item,
      "insert_units"
    );
    const explodability = libredwg.dwg_dynapi_entity_data(
      item,
      "explodable"
    );
    const scalability = libredwg.dwg_dynapi_entity_data(
      item,
      "block_scaling"
    );
    ...
    if (!entities || entities.length == 0) {
      entities = [];
      const entities_ptr = libredwg.dwg_dynapi_entity_data(
        item,
        "entities"
      );
      if (entities_ptr) {
  ...
  convertDimStyle(item, obj) {
    ...
    const DIMTFILLCLR = libredwg.dwg_dynapi_entity_data(
      item,
      "DIMTFILLCLR"
    );
    ...
    const DIMTXTDIRECTION = libredwg.dwg_dynapi_entity_data(
      item,
      "DIMTXTDIRECTION"
    );
    ...
    const DIMTXSTY_Ptr = libredwg.dwg_dynapi_entity_data(
      item,
      "DIMTXSTY"
    );
    const DIMTXSTY = libredwg.dwg_ref_get_absref(DIMTXSTY_Ptr) ?? void 0;
    const DIMLDRBLK_Ptr = libredwg.dwg_dynapi_entity_data(
      item,
      "DIMLDRBLK"
    );
  ...
  convertLayer(item, obj) {
    ...
    const frozenInNew = libredwg.dwg_dynapi_entity_data(
      item,
      "frozen_in_new"
    );
    const locked = libredwg.dwg_dynapi_entity_data(
      item,
      "plotflockedlag"
    );
  ...
  convertLineType(item, obj) {
    ...
    const patternLen = libredwg.dwg_dynapi_entity_data(
      item,
      "pattern_len"
    );
    const dashes = libredwg.dwg_dynapi_entity_data(
      item,
      "dashes"
    );
  ...
  convertStyle(item, obj) {
    ...
    const widthFactor = libredwg.dwg_dynapi_entity_data(
      item,
      "width_factor"
    );
    const obliqueAngle = libredwg.dwg_dynapi_entity_data(
      item,
      "oblique_angle"
    );
    const textGenerationFlag = libredwg.dwg_dynapi_entity_data(
      item,
      "generation"
    );
    const lastHeight = libredwg.dwg_dynapi_entity_data(
      item,
      "last_height"
    );
    const font = libredwg.dwg_dynapi_entity_data(item, "font_file");
    const bigFont = libredwg.dwg_dynapi_entity_data(
      item,
      "bigfont_file"
    );
    return {
  ...
  convertViewport(item, obj) {
    ...
    const aspectRatio = libredwg.dwg_dynapi_entity_data(
      item,
      "aspect_ratio"
    );
    const center = libredwg.dwg_dynapi_entity_data(item, "VIEWCTR");
    const viewTarget = libredwg.dwg_dynapi_entity_data(
      item,
      "view_target"
    );
    const viewDirectionFromTarget = libredwg.dwg_dynapi_entity_data(
      item,
      "VIEWDIR"
    );
    const viewTwistAngle = libredwg.dwg_dynapi_entity_data(
      item,
      "view_twist"
    );
    const lensLength = libredwg.dwg_dynapi_entity_data(
      item,
      "lens_length"
    );
    const frontClippingPlane = libredwg.dwg_dynapi_entity_data(
      item,
      "front_clip_z"
    );
    const backClippingPlane = libredwg.dwg_dynapi_entity_data(
      item,
      "back_clip_z"
    );
    const viewMode = libredwg.dwg_dynapi_entity_data(item, "VIEWMODE");
    const renderMode = libredwg.dwg_dynapi_entity_data(
      item,
      "render_mode"
    );
    const isDefaultLightingOn = libredwg.dwg_dynapi_entity_data(item, "use_default_lights") != 0;
    const defaultLightningType = libredwg.dwg_dynapi_entity_data(
      item,
      "default_lightning_type"
    );
    const brightness = libredwg.dwg_dynapi_entity_data(
      item,
      "brightness"
    );
    const contrast = libredwg.dwg_dynapi_entity_data(item, "contrast");
    const ambient_color = libredwg.dwg_dynapi_entity_data(
      item,
      "ambient_color"
    );
    const lowerLeftCorner = libredwg.dwg_dynapi_entity_data(
      item,
      "lower_left"
    );
    const upperRightCorner = libredwg.dwg_dynapi_entity_data(
      item,
      "upper_right"
    );
    const circleSides = libredwg.dwg_dynapi_entity_data(
      item,
      "circle_zoom"
    );
    const ucsIconSetting = libredwg.dwg_dynapi_entity_data(
      item,
      "UCSICON"
    );
    const gridSpacing = libredwg.dwg_dynapi_entity_data(
      item,
      "GRIDUNIT"
    );
    const snapRotationAngle = libredwg.dwg_dynapi_entity_data(
      item,
      "SNAPANG"
    );
    const snapBasePoint = libredwg.dwg_dynapi_entity_data(
      item,
      "SNAPBASE"
    );
    const snapSpacing = libredwg.dwg_dynapi_entity_data(
      item,
      "SNAPUNIT"
    );
    const ucsOrigin = libredwg.dwg_dynapi_entity_data(
      item,
      "ucsorg"
    );
    const ucsXAxis = libredwg.dwg_dynapi_entity_data(
      item,
      "ucsxdir"
    );
    const ucsYAxis = libredwg.dwg_dynapi_entity_data(
      item,
      "ucsydir"
    );
    const elevation = libredwg.dwg_dynapi_entity_data(
      item,
      "ucs_elevation"
    );
    const majorGridLines = libredwg.dwg_dynapi_entity_data(
      item,
      "grid_major"
    );
    const background = libredwg.dwg_dynapi_entity_data(
      item,
      "background"
    );
    const backgroundObjectId = background ? libredwg.dwg_ref_get_id(background) ?? "" : void 0;
    const visualstyle = libredwg.dwg_dynapi_entity_data(
      item,
      "visualstyle"
    );
  ...
  convertDictionary(item, obj) {
    ...
    const isHardOwner = libredwg.dwg_dynapi_entity_data(
      item,
      "is_hardowner"
    );
    ...
    const itemhandles_ptr = libredwg.dwg_dynapi_entity_data(
      item,
      "itemhandles"
    );
  ...
  convertImageDef(item, obj) {
    ...
    const sizeOfOnePixel = libredwg.dwg_dynapi_entity_data(
      item,
      "pixel_size"
    );
    const resolutionUnits = libredwg.dwg_dynapi_entity_data(
      item,
      "resunits"
    );
    return {
  ...
  convertLayerIndex(item, obj) {
    const libredwg = this.libredwg;
    const commonAttrs = this.getCommonObjectAttrs(obj);
    const timeStamp = libredwg.dwg_dynapi_entity_data(
      item,
      "last_updated"
    );
  ...
  convertLayout(item, obj) {
    const libredwg = this.libredwg;
    const commonAttrs = this.getCommonObjectAttrs(obj);
    const layoutName = libredwg.dwg_dynapi_entity_data(
      item,
      "layout_name"
    );
    const tabOrder = libredwg.dwg_dynapi_entity_data(item, "tab_order");
    const controlFlag = libredwg.dwg_dynapi_entity_data(
      item,
      "layout_flags"
    );
    const insertionPoint = libredwg.dwg_dynapi_entity_data(
      item,
      "INSBASE"
    );
    ...
    const orthographicType = libredwg.dwg_dynapi_entity_data(
      item,
      "UCSORTHOVIEW"
    );
    const minExtent = libredwg.dwg_dynapi_entity_data(
      item,
      "EXTMIN"
    );
    const maxExtent = libredwg.dwg_dynapi_entity_data(
      item,
      "EXTMAX"
    );
    const elevation = libredwg.dwg_dynapi_entity_data(
    ...
    const block_header_ref = libredwg.dwg_dynapi_entity_data(
      item,
      "block_header"
    );
    const paperSpaceTableId = libredwg.dwg_ref_get_id(block_header_ref) ?? "";
    const active_viewport_ref = libredwg.dwg_dynapi_entity_data(
      item,
      "active_viewport"
    );
    const viewportId = libredwg.dwg_ref_get_id(active_viewport_ref) ?? "";
    const named_ucs_ref = libredwg.dwg_dynapi_entity_data(
      item,
      "named_ucs"
    );
  ...
  convertSpatialFilter(item, obj) {
    ...
    const clip_verts_ptr = libredwg.dwg_dynapi_entity_data(
      item,
      "clip_verts"
    );
    ...
    const extrusionDirection = libredwg.dwg_dynapi_entity_data(
      item,
      "extrusion"
    );
    const clipBoundaryVisible = libredwg.dwg_dynapi_entity_data(
      item,
      "display_boundary_on"
    );
    const frontClippingPlaneFlag = libredwg.dwg_dynapi_entity_data(
      item,
      "front_clip_on"
    );
    const frontClippingPlaneDistance = libredwg.dwg_dynapi_entity_data(
      item,
      "front_clip_z"
    );
    const backClippingPlaneFlag = libredwg.dwg_dynapi_entity_data(
      item,
      "back_clip_on"
    );
    const backClippingPlaneDistance = libredwg.dwg_dynapi_entity_data(
      item,
      "back_clip_z"
    );
    const transform_ptr = libredwg.dwg_dynapi_entity_data(
      item,
      "transform"
    );
    const matrix = libredwg.dwg_ptr_to_double_array(transform_ptr, 12);
    const inverse_transform_ptr = libredwg.dwg_dynapi_entity_data(
      item,
      "inverse_transform"
    );
...
const _LibreDwg = class _LibreDwg {
  ...
  dwg_entity_get_dimstyle_name(ptr, field = "dimstyle") {
    ...
    const dimstyle_name = this.dwg_dynapi_entity_data(
      dimstyle_tio,
      "name"
    );
  ...
  dwg_entity_block_header_get_block(ptr) {
    ...
    const base_pt = this.dwg_dynapi_entity_data(
      block_tio,
      "base_pt"
    );
    return {

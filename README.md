...
若 AI 只有终端权限，使用本机 [Agent 模式](docs/AGENT-MODE.zh-CN.md)。运行 `npm run agent`，打开 `http://127.0.0.1:667/agent`；AI 可先 GET `/api/agent` 速读入口，再用 `npm run agent:cli -- docs start`、`sessions`、`state`、`call`、`import`、`save` 和 `export` 操作当前标签页。Agent 模式显式启用同源本机桥；普通静态页面仍不依赖 Node 服务。

...
当前定位事实源：世界坐标固定且不提供可见编辑入口；界面只显示一个“参考锚点”，内部对应可撤销的 `referenceSystem.workFrame`。新特征在 `placement.frameSnapshot` 冻结所用锚点，历史尺寸编辑沿该快照重建，不能读取后来变化的当前锚点。新增模型仅在占据锚点位置时推动它沿 Z 向上避让，未相交则保持不动。独立创建还需明确 `sourceAnchor`。工程格式 v2 存储工作基准、具名基准、对象锚点与特征定位；v1 读取沿用旧世界坐标语义，旧程序应拒绝未知新格式。页面 API v1.12.0 是静态 IIS 运行边界；兼容 CLI/MCP 不是必需服务。验收和未通过项见 `agent/output`，覆盖报告由注册表生成。

> 当前规范：2026-09-23。本文是产品目的、架构边界与验收口径的唯一主说明。
> 原有 M0/M1/M2A、MCP、CLI 记录是实现历史，出现冲突时以本文和实际运行契约为准。

2026-09-26 工具闭环扩展沿现有静态页面、CommandService 和 Worker 实施。`sketchProfile` 保存解析图元或单一参数基础轮廓定义，后续偏移、修复、投影、拉伸加工保持来源与定位快照；孔向导和受限拔模各为一个原子几何步骤。新增检查与临时显示不修改工程。测试、浏览器实测和未执行项分别记录，不以工具卡数量代表验收完成。

2026-09-26 机械 CAD 常用特征扩展：保存来源的 `profileRevolve/profileSweep/profileLoft` 支持新建、加料、切除和交集，保留截面/路径，只替换明确末位加工目标。`profileConstraints` 用有界直线/圆求解器驱动尺寸与关系，读回实际残差、局部 Jacobian rank 和剩余自由度；`inspectConstraints` 只读返回实际来源图元和关系。`offsetSolid/offsetSurface/draftByPlane` 使用精确解析几何，`helix/coil/thread` 创建精确螺旋路径、圆线弹簧和真实 V 槽螺纹；V 槽不冒充标准规格或配合公差。九个原菜单页、原组序和原按钮相对顺序保留，编辑/创建/模型保持展开；原旋转/扫掠/放样入口增加保存来源模式，新特征附加在现有相关组。限制与主流软件对标见 `agent/output/mainstream-cad-tool-audit.md`，不得据此宣称装配配合、工程图、一般自由曲面、钣金或 CAM 已齐全。
...
首次入口统一为 `automation/agent-start.html` / `agent-start.json`，通过 HTML 元数据、llms.txt、既有 AGENT 接口说明和 `connect().onboarding` 发现。先查看宿主实际能力并读取通道文档；标签页绑定失败不能代表页面 API 缺失。可选本地技能包提供操作说明、连接脚本助手和真实 UI/API 路由，Windows 安装器由具备磁盘能力的宿主查看后执行，逐文件验哈希并备份旧技能。网页不自动安装，也不增加服务或权限。
...
## 2026-09-26 快捷五金模型和面加工

新增弹簧、螺丝、丝筒、半圆钉四个参数化快捷模型，保留原模型 kind 并按简单到复杂排列。模型菜单五个常用项按成功创建次数排序，完整目录仍在快捷模型面板；列表最小 240px，空间不足向下滚动。螺丝有两种头型、四种槽型、固定沉头 0.20 mm 直升位和真实 V 牙槽；丝筒向本地 -Z 插入，从下往上攻牙。新增锣槽、内车、外车，使用一个选中的平面及明确尺寸向实体内切除。页面 API 1.13.0 覆盖所有新增功能和只读频率查询。完整契约、坐标语义和限制见 docs/PAGE-API.zh-CN.md 的快捷五金模型章节。其余菜单设计和展开策略保持。

用户确认：锣槽、内车、外车属于基本加工，均在加工菜单直接展开，不列为快捷模型、不放入更多折叠项。

...
test('version 1 identity survives native JSON save/reopen; identity-less legacy input gets one',()=>{
 ...
 const ctx={crypto};vm.createContext(ctx);vm.runInContext(source.slice(start,end)+'this.validate=validateDocument;',ctx);
 const legacy={version:1,name:'Legacy',features:[{id:'b',op:'box',params:{width:50,depth:30,height:3},refs:[]}],imports:{},hidden:[]};
 const first=ctx.validate(legacy);assert.equal(typeof first.documentId,'string');assert(!Object.hasOwn(legacy,'documentId'));

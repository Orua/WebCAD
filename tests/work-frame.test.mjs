...
const anchored=applyReferenceAction(referenceSystem,'reference.setBodyAnchor',{bodyId:'plate',name:'定位孔',referenceId:'verified',quaternion:[0,0,0,1]},{bodyId:'plate',worldPoint:[101,51,6],geometryFingerprint:'brep-sha256:fixture'});
const anchorId=anchored.bodyAnchors[0].anchorId;
assert.equal(validateReferenceSystem(anchored).bodyAnchors[0].anchorId,anchorId);
const namedPlacement=resolvePlacement({version:1,frame:{kind:'world'},sourceAnchor:{kind:'named',anchorId}},anchored,'transform',{mode:'toPoint',targetPoint:[200,0,0],orientation:'preserve'});
assert.deepEqual(namedPlacement.sourcePoint,[101,51,6]);
assert.deepEqual(validateResolvedPlacement(namedPlacement,'transform'),namedPlacement);
assert.throws(()=>resolvePlacement({version:1,frame:{kind:'world'},sourceAnchor:{kind:'named',anchorId:'missing'}},anchored,'transform',{mode:'toPoint',targetPoint:[200,0,0]}),{code:'STALE_REFERENCE'});

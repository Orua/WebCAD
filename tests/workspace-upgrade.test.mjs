...
test('all configurable header/tab/control actions have public routes',()=>{
 assert.deepEqual(UI_LAYOUT.tabs.map(tab=>tab.id),['create','model','edit','machine','finish','surface','inspect','view','settings']);assert(Object.isFrozen(UI_LAYOUT.tabs));
 ...
 for(const control of Object.values(UI_LAYOUT.controls))for(const [,value]of control.items)assert(UI_LAYOUT.icons[value]?.length,`missing view icon: ${value}`);
});

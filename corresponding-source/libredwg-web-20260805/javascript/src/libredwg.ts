...
/* Modified by Golden Luck Hardware on 2026-08-05 for large-file browser decoding and cleanup. */
...
export class LibreDwg {
  ...
  dwg_entity_get_dimstyle_name(
    ...
    const dimstyle_name = this.dwg_dynapi_entity_data<string>(
      dimstyle_tio,
      'name'
    )
  ...
  dwg_entity_block_header_get_block(
    ...
    const base_pt = this.dwg_dynapi_entity_data<DwgPoint2D>(
      block_tio,
      'base_pt'
    )
    return {
  ...
  dwg_entity_insert_get_attribs(ptr: Dwg_Object_Ptr): number[] {
    const wasmInstance = this.wasmInstance
    return wasmInstance.dwg_entity_insert_get_attribs(ptr).data as number[]
  }
  ...
  dwg_object_dictionary_get_texts(ptr: Dwg_Object_Ptr): string[] {
    const wasmInstance = this.wasmInstance
    return wasmInstance.dwg_object_dictionary_get_texts(ptr).data as string[]
  }

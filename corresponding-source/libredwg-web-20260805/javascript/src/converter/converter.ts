...
/* Modified by Golden Luck Hardware on 2026-08-05 for bounded, progressive viewer conversion. */
export interface ViewerEntityBatch {
  kind: 'block' | 'model'
  blockName: string
  blockHandle: string
...
export interface ViewerConversionSummary {
  entityCount: number
  blockCount: number
  layers: DwgLayerTableEntry[]
  textStyles: DwgStyleTableEntry[]
  stats: ReturnType<LibreDwgConverter['getConversionStats']>
}
...
export class LibreDwgConverter {
  ...
  /**
   * Converts only data required by a read-only viewer and emits entities in bounded batches.
  ...
  convertForViewer(
    data: Dwg_Data_Ptr,
    onBatch: (batch: ViewerEntityBatch) => void,
    batchSize = 2000
  ): ViewerConversionSummary {
    this.entityConverter.clear()
    const libredwg = this.libredwg
    const classes: DwgClass[] = []
    const layers: DwgLayerTableEntry[] = []
    const textStyles: DwgStyleTableEntry[] = []
    const blocks: Array<{
      obj: Dwg_Object_Ptr
    ...
    for (let i = 0; i < numObjects; i++) {
      ...
      if (!tio) continue
      const fixedtype = libredwg.dwg_object_get_fixedtype(obj)
      if (fixedtype === Dwg_Object_Type.DWG_TYPE_LAYER) {
        const layer = this.convertLayer(tio, obj)
        layers.push(layer)
        this.entityConverter.layers.set(layer.handle, layer.name)
        ...
        const name = block.name || attrs.name
        if (isPaperSpace(name)) continue
        blocks.push({
          obj,
          tio,
          name,
          handle: attrs.handle,
          basePoint:
            libredwg.dwg_dynapi_entity_data<DwgPoint3D>(tio, 'base_pt') ??
            ({ x: 0, y: 0, z: 0 } as DwgPoint3D),
          numOwned:
            libredwg.dwg_dynapi_entity_data<number>(tio, 'num_owned') ?? 0
        })
      }
    }

    // Emit model space first so a viewer can paint the drawing's coarse geometry immediately.
    // INSERT references are resolved after the later block-definition batches arrive.
    blocks.sort(
      (a, b) => Number(isModelSpace(b.name)) - Number(isModelSpace(a.name))
    )

    let entityCount = 0
    const emitBlock = (block: (typeof blocks)[number]) => {
      let batch: DwgEntity[] = []
      let blockEntityCount = 0
      const kind = isModelSpace(block.name) ? 'model' : 'block'
      const emit = () => {
      ...
      const append = (obj: Dwg_Object_Ptr) => {
        const entity = this.entityConverter.convert(obj)
        if (!entity) return
        entity.ownerBlockRecordSoftId = block.handle
        batch.push(entity)
        blockEntityCount++
        if (batch.length >= safeBatchSize) emit()
      }

      let next = libredwg.get_first_owned_entity(block.obj)
      while (next) {
        append(next)
        next = libredwg.get_next_owned_entity(block.obj, next)
      }

      // Some LibreDWG versions fail to expose the owned-entity iterator. Use the block's
      // object-reference array as the same fallback as the full converter.
      if (blockEntityCount === 0 && block.numOwned > 0) {
        const entitiesPtr = libredwg.dwg_dynapi_entity_data<number>(
          block.tio,
          'entities'
        )
        if (entitiesPtr) {
          const refs = libredwg.dwg_ptr_to_object_ref_ptr_array(
            entitiesPtr,
            block.numOwned
          )
          for (let i = 0; i < block.numOwned; i++) {
            const obj = libredwg.dwg_ref_get_object(refs[i])
            if (obj) append(obj)
          }
        }
      }
  ...
  private convertHeader(data: Dwg_Data_Ptr, header: DwgHeader) {
    ...
    HEADER_VARIABLES.forEach(name => {
      ...
      let value = libredwg.dwg_dynapi_header_data<number | string>(
        data,
        var_name
      )
  ...
  private convertBlockRecord(
    ...
    const description = libredwg.dwg_dynapi_entity_data<string>(
      item,
      'description'
    )
    const basePoint = libredwg.dwg_dynapi_entity_data<DwgPoint3D>(
      item,
      'base_pt'
    )
    const insertionUnits = libredwg.dwg_dynapi_entity_data<number>(
      item,
      'insert_units'
    )
    const explodability = libredwg.dwg_dynapi_entity_data<number>(
      item,
      'explodable'
    )
    const scalability = libredwg.dwg_dynapi_entity_data<number>(
      item,
      'block_scaling'
    )
    ...
    if (!entities || entities.length == 0) {
      entities = []
      const entities_ptr = libredwg.dwg_dynapi_entity_data<number>(
        item,
        'entities'
      )
      if (entities_ptr) {
  ...
  private convertDimStyle(
    ...
    const DIMTFILLCLR = libredwg.dwg_dynapi_entity_data<number>(
      item,
      'DIMTFILLCLR'
    )
    ...
    const DIMTXTDIRECTION = libredwg.dwg_dynapi_entity_data<number>(
      item,
      'DIMTXTDIRECTION'
    )
    ...
    const DIMTXSTY_Ptr = libredwg.dwg_dynapi_entity_data<number>(
      item,
      'DIMTXSTY'
    )
    const DIMTXSTY = libredwg.dwg_ref_get_absref(DIMTXSTY_Ptr) ?? undefined
    const DIMLDRBLK_Ptr = libredwg.dwg_dynapi_entity_data<number>(
      item,
      'DIMLDRBLK'
    )
  ...
  private convertLayer(
    ...
    const frozenInNew = libredwg.dwg_dynapi_entity_data<number>(
      item,
      'frozen_in_new'
    )
    const locked = libredwg.dwg_dynapi_entity_data<number>(
      item,
      'plotflockedlag'
    )
  ...
  private convertLineType(
    ...
    const patternLen = libredwg.dwg_dynapi_entity_data<number>(
      item,
      'pattern_len'
    )
    const dashes = libredwg.dwg_dynapi_entity_data<Dwg_Array_Ptr>(
      item,
      'dashes'
    )
  ...
  private convertStyle(
    ...
    const widthFactor = libredwg.dwg_dynapi_entity_data<number>(
      item,
      'width_factor'
    )
    const obliqueAngle = libredwg.dwg_dynapi_entity_data<number>(
      item,
      'oblique_angle'
    )
    const textGenerationFlag = libredwg.dwg_dynapi_entity_data<number>(
      item,
      'generation'
    )
    const lastHeight = libredwg.dwg_dynapi_entity_data<number>(
      item,
      'last_height'
    )
    const font = libredwg.dwg_dynapi_entity_data<string>(item, 'font_file')
    const bigFont = libredwg.dwg_dynapi_entity_data<string>(
      item,
      'bigfont_file'
    )
  ...
  private convertViewport(
    ...
    const aspectRatio = libredwg.dwg_dynapi_entity_data<number>(
      item,
      'aspect_ratio'
    )
    const center = libredwg.dwg_dynapi_entity_data<DwgPoint2D>(item, 'VIEWCTR')
    const viewTarget = libredwg.dwg_dynapi_entity_data<DwgPoint3D>(
      item,
      'view_target'
    )
    const viewDirectionFromTarget = libredwg.dwg_dynapi_entity_data<DwgPoint3D>(
      item,
      'VIEWDIR'
    )
    const viewTwistAngle = libredwg.dwg_dynapi_entity_data<number>(
      item,
      'view_twist'
    )
    const lensLength = libredwg.dwg_dynapi_entity_data<number>(
      item,
      'lens_length'
    )
    const frontClippingPlane = libredwg.dwg_dynapi_entity_data<number>(
      item,
      'front_clip_z'
    )
    const backClippingPlane = libredwg.dwg_dynapi_entity_data<number>(
      item,
      'back_clip_z'
    )
    const viewMode = libredwg.dwg_dynapi_entity_data<number>(item, 'VIEWMODE')
    const renderMode = libredwg.dwg_dynapi_entity_data<number>(
      item,
      'render_mode'
    )
    const isDefaultLightingOn =
      libredwg.dwg_dynapi_entity_data<number>(item, 'use_default_lights') != 0
    const defaultLightningType = libredwg.dwg_dynapi_entity_data<number>(
      item,
      'default_lightning_type'
    )
    const brightness = libredwg.dwg_dynapi_entity_data<number>(
      item,
      'brightness'
    )
    const contrast = libredwg.dwg_dynapi_entity_data<number>(item, 'contrast')
    const ambient_color = libredwg.dwg_dynapi_entity_data<Dwg_Color>(
      item,
      'ambient_color'
    )

    // ViewportTableRecord
    const lowerLeftCorner = libredwg.dwg_dynapi_entity_data<DwgPoint2D>(
      item,
      'lower_left'
    )
    const upperRightCorner = libredwg.dwg_dynapi_entity_data<DwgPoint2D>(
      item,
      'upper_right'
    )
    // TODO: Not sure whether 'circleSides' is equal to 'circle_zoom'
    const circleSides = libredwg.dwg_dynapi_entity_data<number>(
      item,
      'circle_zoom'
    )
    const ucsIconSetting = libredwg.dwg_dynapi_entity_data<number>(
      item,
      'UCSICON'
    )
    // TODO: Not sure whether 'gridSpacing' is equal to 'GRIDUNIT'
    const gridSpacing = libredwg.dwg_dynapi_entity_data<DwgPoint2D>(
      item,
      'GRIDUNIT'
    )
    const snapRotationAngle = libredwg.dwg_dynapi_entity_data<number>(
      item,
      'SNAPANG'
    )
    const snapBasePoint = libredwg.dwg_dynapi_entity_data<DwgPoint2D>(
      item,
      'SNAPBASE'
    )
    // TODO: Not sure whether 'snapSpacing' is equal to 'SNAPUNIT'
    const snapSpacing = libredwg.dwg_dynapi_entity_data<DwgPoint2D>(
      item,
      'SNAPUNIT'
    )
    const ucsOrigin = libredwg.dwg_dynapi_entity_data<DwgPoint3D>(
      item,
      'ucsorg'
    )
    const ucsXAxis = libredwg.dwg_dynapi_entity_data<DwgPoint3D>(
      item,
      'ucsxdir'
    )
    const ucsYAxis = libredwg.dwg_dynapi_entity_data<DwgPoint3D>(
      item,
      'ucsydir'
    )
    const elevation = libredwg.dwg_dynapi_entity_data<number>(
      item,
      'ucs_elevation'
    )
    const majorGridLines = libredwg.dwg_dynapi_entity_data<number>(
      item,
      'grid_major'
    )
    const background = libredwg.dwg_dynapi_entity_data<Dwg_Object_Ref_Ptr>(
      item,
      'background'
    )
    const backgroundObjectId = background
      ...
      : undefined
    const visualstyle = libredwg.dwg_dynapi_entity_data<Dwg_Object_Ref_Ptr>(
      item,
      'visualstyle'
    )
  ...
  private convertDictionary(
    ...
    const isHardOwner = libredwg.dwg_dynapi_entity_data<number>(
      item,
      'is_hardowner'
    )
    ...
    const itemhandles_ptr = libredwg.dwg_dynapi_entity_data<number>(
      item,
      'itemhandles'
    )
  ...
  private convertImageDef(
    ...
    const sizeOfOnePixel = libredwg.dwg_dynapi_entity_data<DwgPoint2D>(
      item,
      'pixel_size'
    )
    const resolutionUnits = libredwg.dwg_dynapi_entity_data<number>(
      item,
      'resunits'
    )
  ...
  private convertLayerFilter(
    ...
    const commonAttrs = this.getCommonObjectAttrs(obj)
    const numNames =
      libredwg.dwg_dynapi_entity_data<number>(item, 'num_names') ?? 0
  ...
  private convertLayerIndex(
    ...
    const libredwg = this.libredwg
    const commonAttrs = this.getCommonObjectAttrs(obj)
    const timeStamp = libredwg.dwg_dynapi_entity_data<number>(
      item,
      'last_updated'
    )
    const numEntries =
    ...
    if (entriesPtr && entrySize > 0) {
      for (let i = 0; i < numEntries; i++) {
        ...
        idBufferIds.push(
          handleRef ? (libredwg.dwg_ref_get_id(handleRef) ?? '') : ''
        )
  ...
  private convertLayout(
    ...
    // AcDbLayout
    const layoutName = libredwg.dwg_dynapi_entity_data<string>(
      item,
      'layout_name'
    )
    const tabOrder = libredwg.dwg_dynapi_entity_data<number>(item, 'tab_order')
    const controlFlag = libredwg.dwg_dynapi_entity_data<number>(
      item,
      'layout_flags'
    )
    const insertionPoint = libredwg.dwg_dynapi_entity_data<DwgPoint3D>(
      item,
      'INSBASE'
    )
    ...
    const orthographicType = libredwg.dwg_dynapi_entity_data<number>(
      item,
      'UCSORTHOVIEW'
    )
    const minExtent = libredwg.dwg_dynapi_entity_data<DwgPoint3D>(
      item,
      'EXTMIN'
    )
    const maxExtent = libredwg.dwg_dynapi_entity_data<DwgPoint3D>(
      item,
      'EXTMAX'
    )
    const elevation = libredwg.dwg_dynapi_entity_data<number>(
    ...
    const block_header_ref = libredwg.dwg_dynapi_entity_data<number>(
      item,
      'block_header'
    )
    const paperSpaceTableId = libredwg.dwg_ref_get_id(block_header_ref) ?? ''

    const active_viewport_ref = libredwg.dwg_dynapi_entity_data<number>(
      item,
      'active_viewport'
    )
    const viewportId = libredwg.dwg_ref_get_id(active_viewport_ref) ?? ''

    const named_ucs_ref = libredwg.dwg_dynapi_entity_data<number>(
      item,
      'named_ucs'
    )
  ...
  private convertMLeaderStyle(
    ...
    const libredwg = this.libredwg
    const commonAttrs = this.getCommonObjectAttrs(obj)
    const objectVal = <T>(field: string) =>
      libredwg.dwg_dynapi_entity_data<T>(item, field)
    ...
    return {
      ...
      blockContentRotationEnabled: asBool(
        objectVal<number>('use_block_rotation')
      ),
  ...
  private convertSpatialFilter(
    ...
    const numberOfPointsOnClipBoundary =
      libredwg.dwg_dynapi_entity_data<number>(item, 'num_clip_verts')
    const clip_verts_ptr = libredwg.dwg_dynapi_entity_data<number>(
      item,
      'clip_verts'
    )
    ...
    const extrusionDirection = libredwg.dwg_dynapi_entity_data<DwgPoint3D>(
      item,
      'extrusion'
    )
    const clipBoundaryVisible = libredwg.dwg_dynapi_entity_data<number>(
      item,
      'display_boundary_on'
    )
    const frontClippingPlaneFlag = libredwg.dwg_dynapi_entity_data<number>(
      item,
      'front_clip_on'
    )
    const frontClippingPlaneDistance = libredwg.dwg_dynapi_entity_data<number>(
      item,
      'front_clip_z'
    )
    const backClippingPlaneFlag = libredwg.dwg_dynapi_entity_data<number>(
      item,
      'back_clip_on'
    )
    const backClippingPlaneDistance = libredwg.dwg_dynapi_entity_data<number>(
      item,
      'back_clip_z'
    )
    const transform_ptr = libredwg.dwg_dynapi_entity_data<number>(
      item,
      'transform'
    )
    const matrix = libredwg.dwg_ptr_to_double_array(transform_ptr, 12)
    const inverse_transform_ptr = libredwg.dwg_dynapi_entity_data<number>(
      item,
      'inverse_transform'
    )
  ...
  private convertXRecord(
    ...
    const extensionDictionary =
      xdic && xdic.absolute_ref ? idToString(xdic.absolute_ref) : undefined
